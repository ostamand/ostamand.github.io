...
In order term, our training environment need to define the states, actions and rewards that define our problem. 

For the states, it consist of eight numbers at each timestep:
...
Two continous actions are allowed: moving to the left and moving to the left. 
...
Finally, the training environment is shown below. Note that to speed up training twelve parallel identical environments were combined. 

![overall](/assets/rcb_training.png)

The training of the agents using PPO is showed in the following clip. For more details on the hyperparameters and neural network architecture used refer to the [github page](https://github.com/ostamand/rolling-balls) of the project.

{% include rcb_youtube_training.html %}

As observed, the agent learns very fast to move quickly to the target location. 
